...
from ..catalog.support import REFUSED, SUPPORTED
...
def _status(class_id: str) -> tuple[str, str]:
    if class_id in SUPPORTED:
        return TICK, ""
    if class_id in REFUSED:
        return CROSS, REFUSED[class_id]
    return CROSS, "no conversion rule"
...
def render(pkg: Package, show_graph: bool = True) -> str:
    ...
    for df in pkg.dataflows:
        ...
        for comp in df.components:
            mark, why = _status(comp.class_id)
